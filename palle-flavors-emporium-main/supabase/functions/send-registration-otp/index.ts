...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!fromEmail) {
      return json({
        error:
          "OTP_FROM_EMAIL is missing. Set it to a verified Resend sender email or domain sender in Supabase Edge Function secrets.",
      }, 500);
    }
    if (fromEmail === "onboarding@resend.dev") {
      return json({
        error:
          "OTP_FROM_EMAIL is still using onboarding@resend.dev. That sender only works for Resend test inboxes. Set a verified sender email or a domain sender to email real users.",
      }, 500);
    }
